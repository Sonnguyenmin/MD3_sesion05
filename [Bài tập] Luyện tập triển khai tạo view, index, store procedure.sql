...
create view all_customer as select * from customer;
select * from all_customer;

-- Tạo view hiển thị tất cả order có oTotalPrice trên 150000
create view all_orders as select * from orders where o_total_price > 150000;
select * from all_orders;

-- Đánh index cho bảng customer ở cột cName
create index IDX_name on customer(c_name);
...
begin
	select * from orders o order by o.o_total_price asc limit 1;  
end;
//
delimiter ;
...
delimiter //
create procedure get_customer_by_product()
begin
	select c.c_name, count(od.od_qty) as number_may_giat
    from customer c
    join orders o on c.c_id = o.c_id
    join order_detail od on o.o_id = od.o_id
    join product p on od.p_id = p.p_id
    ...
    group by c.c_name
    order by number_may_giat asc
    limit 1;
end;
//
delimiter ;
    
call get_customer_by_product();